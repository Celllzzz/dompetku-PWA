...
export default function Home() {
  ...
  return (
    <div className="p-6 space-y-8 animate-fade-in">
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-blue-200">
      ...
      <div className="sticky top-0 bg-gray-50 pt-2 pb-2 z-10">
        <div className="flex gap-3">
          <div className="flex-1 bg-white group focus-within:ring-2 focus-within:ring-blue-500/20 flex items-center px-4 py-3.5 rounded-2xl border border-gray-200 shadow-sm transition-all duration-300">
            <Search size={20} className="text-gray-400 mr-3 group-focus-within:text-blue-500 transition-colors" />
            <input 
              type="text" placeholder="Cari transaksi..." className="bg-transparent outline-none text-sm w-full font-medium"
          ...
          <button onClick={() => setShowFilter(!showFilter)} className={`p-3.5 rounded-2xl border shadow-sm transition-all duration-300 ${showFilter ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-500 border-gray-200'}`}><Filter size={20} /></button>
        </div>
        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showFilter ? 'max-h-20 opacity-100 mt-3' : 'max-h-0 opacity-0'}`}>
          <select className="w-full p-3.5 bg-white border border-gray-200 rounded-2xl text-sm font-medium focus:outline-none focus:border-blue-500" value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)}>
      ...
      <div className="space-y-4">
        ...
        {loading ? <p className="text-center text-gray-400 py-10">Loading...</p> : filteredTransactions.length === 0 ? <p className="text-center text-gray-400 py-10">Tidak ada transaksi</p> : filteredTransactions.map((trx, index) => (
            <div key={trx.id} style={{ animationDelay: `${index * 100}ms` }} onClick={() => navigate(`/transaction/${trx.id}`)} className="group bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex justify-between items-center cursor-pointer hover:border-blue-200 animate-slide-up">
              <div className="flex items-center gap-5">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl font-bold ${trx.type === 'income' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>{trx.categories?.name ? trx.categories.name.charAt(0).toUpperCase() : '?'}</div>
                <div>
                  <h4 className="font-bold text-gray-800 text-base mb-1">{trx.title}</h4>
                  <p className="text-xs text-gray-400">{trx.categories?.name} • {new Date(trx.date).toLocaleDateString('id-ID')}</p>
                </div>
              </div>
              <div className="text-right"><span className={`block font-bold text-sm ${trx.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>{trx.type === 'income' ? '+' : '-'} {formatRupiah(trx.amount)}</span></div>
            </div>
        ))}
      </div>
    </div>
