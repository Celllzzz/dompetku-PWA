...
import { Home, List, PlusCircle, User, Wallet, LogOut, PieChart } from 'lucide-react';
...
export default function Layout({ children }) {
  ...
  const menus = [
    ...
    { path: '/history', icon: PieChart, label: 'History' }, // <--- MENU BARU
    ...
    <div className="flex h-screen w-full bg-gray-50 overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-72 bg-white border-r border-gray-200 h-full p-6 shadow-sm z-20 flex-shrink-0">
        <div className="flex items-center gap-3 mb-10 px-2 animate-fade-in">
          <div className="p-2.5 bg-blue-600 rounded-xl text-white shadow-lg shadow-blue-200 hover:scale-105 transition-transform">
            <Wallet size={28} />
          </div>
          <div>
        ...
        <nav className="md:hidden absolute bottom-0 w-full bg-white/80 backdrop-blur-xl border-t border-gray-200/50 flex justify-around items-center pb-safe z-50 shadow-[0_-5px_20px_rgba(0,0,0,0.03)] h-[80px]">
          {menus.map((item) => {
            ...
            return (
              ...
              >
                {active && (
                  <span className="absolute top-0 w-8 h-1 bg-blue-600 rounded-b-lg shadow-[0_0_10px_rgba(37,99,235,0.5)] transition-all duration-300 animate-fade-in" />
                )}
                <div className={`p-1.5 rounded-2xl transition-all duration-300 ${active ? 'text-blue-600 -translate-y-1' : 'text-gray-400 group-hover:text-gray-600'}`}>
                  <item.icon size={24} strokeWidth={active ? 2.5 : 2} />
                </div>
                <span className={`text-[10px] font-medium transition-all duration-300 ${active ? 'text-blue-600 opacity-100' : 'text-gray-400 opacity-0 h-0 overflow-hidden group-hover:opacity-100 group-hover:h-auto'}`}>
                  {item.label}
                </span>
              </Link>
            );
